var backendLessModule = angular.module('backendLessModule',['ngMockE2E']);

backendLessModule.run(function(backendLess) {
    backendLess.setUp();
});


backendLessModule.provider('backendLess', function backendLessProvider() {
    this.$get = function ($httpBackend, baseAPIRoute) {
        return new backendLess($httpBackend, baseAPIRoute);
    };
});


function backendLess($httpBackend, baseAPIRoute) {
    ...
    function setUp () {
        $httpBackend.whenGET(new RegExp('^'+baseAPIRoute+'\/tasks\\?limit=\\d+&offset=\\d+')).respond(fetchTasks);
        $httpBackend.whenGET(new RegExp('^'+baseAPIRoute+'\/tasks\/\\d+')).respond(fetchTask);
        $httpBackend.whenPUT(new RegExp('^'+baseAPIRoute+'\/tasks\/\\d+\/done')).respond(completeTask);
        $httpBackend.whenPUT(new RegExp('^'+baseAPIRoute+'\/tasks\/\\d+')).respond(updateTask);
        $httpBackend.whenPATCH(new RegExp('^'+baseAPIRoute+'\/tasks\/\\d+')).respond(updateTask);
        $httpBackend.whenDELETE(new RegExp('^'+baseAPIRoute+'\/tasks\/\\d+')).respond(deleteTask);
        $httpBackend.whenPOST(new RegExp('^'+baseAPIRoute+'\/tasks')).respond(addTask);
    ...
    function completeTask (method, url) {
        var id = url.replace(baseAPIRoute+'/tasks/','').replace('/done','').replace('/','');
    ...
    function deleteTask (method, url) {
        var id = url.replace(baseAPIRoute+'/tasks/','').replace('/');
